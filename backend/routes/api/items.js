...
router.get("/", auth.optional, function (req, res, next) {
    ...
    Promise.all([req.query.seller ? User.findOne({username: req.query.seller}) : null, req.query.favorited ? User.findOne({username: req.query.favorited}) : null])
        .then(function (results) {
            ...
            return Promise.all([Item.find(query)
                .limit(Number(limit))
                .skip(Number(offset))
                .sort({createdAt: "desc"})
                .exec(), Item.count(query).exec(), req.payload ? User.findById(req.payload.id) : null]).then(async function (results) {
                var items = results[0];
...
router.get("/feed", auth.required, function (req, res, next) {
    ...
    User.findById(req.payload.id).then(function (user) {
        ...
        Promise.all([Item.find({seller: {$in: user.following}})
            .limit(Number(limit))
            .skip(Number(offset))
            .populate("seller")
            .exec(), Item.count({seller: {$in: user.following}})])
            .then(function (results) {
                var items = results[0];
                var itemsCount = results[1];
